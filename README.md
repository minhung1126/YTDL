...
2. 在最新版本的 "Assets" 區塊中下載您需要的版本：
    - `YTDL.<版本號>.Windows.zip` — 含執行檔與完整執行環境 (yt-dlp、FFmpeg、Deno)，解壓縮即可使用。
    - `YTDL.<版本號>.zip` — 原始碼，需自行安裝 Python 與相依套件。
...
若專案根目錄存在 `.gitignore` 檔案 (即開發環境)，`__version__` 會自動設為 `"dev"`，以避免觸發自動更新機制，無需額外建立任何檔案。
...
2. 推送 tag 即可觸發 GitHub Actions 自動建立 Release:
    ...
    ```bash
    git tag v2026.02.21.01
    git push origin v2026.02.21.01
    ```
